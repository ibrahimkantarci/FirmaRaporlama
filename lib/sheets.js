...
// Sekmedeki tüm müşteri ID'lerini (benzersiz) döndürür.
export async function listSheetCustomers({ tab } = {}) {
  const values = await readMatrixFromSheet({ tab });
  const seen = new Set();
  const ids = [];
  for (const row of values) {
    const id = parseMetaId(row?.[0]);
