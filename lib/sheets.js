...
// Harici (paylaşılan) bir spreadsheet'ten sekme okur — servis hesabının o dosyada
// en az Görüntüleyen olması gerekir. renewal_data gibi canlı güncellenen dış
// kaynaklar için; ana GOOGLE_SHEET_ID'ye dokunmaz.
export async function readMatrixFromExternalSheet({ spreadsheetId, tab, tabMatch }) {
  if (!spreadsheetId) throw new Error("spreadsheetId gerekli.");
  const auth = getAuth();
  const sheets = google.sheets({ version: "v4", auth });
  const read = async (t) => {
    const res = await sheets.spreadsheets.values.get({
  ...
  if (tab) {
    try { return await read(tab); } catch (e) { if (!tabMatch) throw e; }
  }
  // Sekme adı birebir tutmadı → başlıkları listeleyip eşleşeni bul (Sheet sahibi adı
  // değiştirirse kırılmasın diye).
  const meta = await sheets.spreadsheets.get({ spreadsheetId, fields: "sheets.properties.title" });
  ...
  const fold = (s) => String(s || "").toLocaleLowerCase("tr-TR")
    ...
    .replace(/\s+/g, " ").trim();
  const want = fold(tabMatch || tab);
  const hit = titles.find((t) => fold(t) === want)
    || titles.find((t) => fold(t).includes(want))
    || titles.find((t) => want.includes(fold(t)));
  if (!hit) throw new Error(`Sekme bulunamadı (${tab || tabMatch}). Mevcut: ${titles.join(", ")}`);
  return await read(hit);
}
