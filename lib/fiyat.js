...
function buildCatalogIndex(catalog) {
  ...
  catalog.rows.forEach((r, ri) => {
    ...
    (((idx[pid] ||= {})[unit] ||= {})[cur] ||= []).push({ price, period, isMain });
  });
  return { idx, pyByProvider, C, missing };
}

// Referansları, kampanyanın dönemine uygun katalog satırlarından hesaplar.
// Dönem eşleşeni yoksa tüm dönemlere düşer (fallback).
function refsFrom(entries, campPeriod) {
  if (!entries || !entries.length) return null;
  let rel = entries.filter((e) => periodMatch(campPeriod, e.period));
  if (!rel.length) rel = entries; // dönem eşleşmedi → tüm dönemler
  const prices = rel.map((e) => e.price);
  const mains = rel.filter((e) => e.isMain).map((e) => e.price);
  return {
    min: Math.min(...prices),
    max: Math.max(...prices),
...
export function buildComparison(catalog, campaign) {
  ...
  campaign.rows.forEach((r, ri) => {
    ...
    const ref = refsFrom(idx[pid]?.[unit]?.[cur], period);
