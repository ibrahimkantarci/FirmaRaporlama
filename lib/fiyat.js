...
// --- Üst Kategori: kategoriden türetilir (Venue / Big 5 / Others) ---
// Varyant yazımlar (ör. "Organizasyon" vs "Organizasyonu") için her ikisi de eklendi.
const _BIG5 = new Set([
  "düğün fotoğrafçıları",
  "düğün organizasyon", "düğün organizasyonu",
  "gelin saçı ve makyajı", "gelin saçı makyajı",
  "gelinlik",
  "kına ve bekarlığa veda organizasyon", "kına ve bekarlığa veda organizasyonu",
  "müzik",
]);
const _OTHERS = new Set([
  "alternatif fikirler",
  "catering hizmetleri", "catering",
  "çiçekçiler",
  "dans kursları",
  "düğün davetiyesi", "davetiye",
  "gelin arabası",
  "evlilik teklifi",
  "nikah şekeri ve hediyelik",
]);
export function upperCategory(category) {
  const c = String(category ?? "").toLocaleLowerCase("tr").replace(/\s+/g, " ").trim();
  if (!c) return "";
  if (_BIG5.has(c)) return "Big 5";
  if (_OTHERS.has(c)) return "Others";
  return "Venue";
}
...
export function buildComparison(catalog, campaign) {
  ...
  campaign.rows.forEach((r, ri) => {
    ...
    rows.push({
      ...
      upperCategory: upperCategory(String(r[M.category] ?? "")),
...
const KIYAS_HEADER = [
  "Provider Id", "Provider Adı", "Kategori", "Üst Kategori", "Şehir", "Sorumlu PY", "Kampanya Id", "Tür", "Etiket",
...
export function parseKiyasSheet(values) {
  ...
  const rows = values.slice(i + 1).filter((r) => r && r.length).map((r) => {
    ...
    return {
      ...
      upperCategory: String(at(r, "Üst Kategori") ?? ""),
...
export function kiyasMatrix(rows, strategy = "max") {
  ...
  const data = rows.map((r) => {
    ...
    return [
      r.providerId, r.providerName, r.category, r.upperCategory ?? "", r.city, r.responsiblePY ?? "", r.campaignId, r.type, r.label,
