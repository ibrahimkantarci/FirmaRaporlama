...
export default function ErceGate({ children }) {
  ...
  const shell = { flex: 1, display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg)" };
  const card = { width: 360, background: "var(--surface)", border: "1px solid var(--line)", borderRadius: 12, padding: 28, boxShadow: "0 1px 3px rgba(0,0,0,0.06)" };
  const input = { width: "100%", boxSizing: "border-box", border: "1px solid var(--line-strong)", borderRadius: 8, padding: "10px 12px", fontSize: 14, marginTop: 8 };
  const btn = { width: "100%", background: "var(--brand)", color: "#fff", border: "none", borderRadius: 8, padding: "10px 16px", fontSize: 14, cursor: "pointer", marginTop: 14 };

  return (
    <div style={shell}>
      <div style={card}>
        {phase === "loading" && <div style={{ color: "var(--muted)" }}>Yükleniyor…</div>}

        {phase === "enter" && (
          <div>
            <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 2 }}>🔒 Erce İçin</div>
            <div style={{ fontSize: 13, color: "var(--muted)", marginBottom: 8 }}>Bu sayfa parolayla korunuyor.</div>
